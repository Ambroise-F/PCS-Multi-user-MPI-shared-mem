...
void lin_comb(point_t * R, mpz_t a, mpz_t b)
{
	...
	double_and_add(&S2, Q, b, E); // S2 = bQ
	add(R, S1, S2, E); // R = aP + bQ
...
void pcs_init(point_t P_init, point_t Q_init, elliptic_curve_t E_init, mpz_t n_init, mpz_t *A_init, mpz_t *B_init, uint8_t nb_bits_init, uint8_t trailling_bits_init, int type_struct, int nb_threads, uint8_t level)
{
	...
	for(i=0; i<__NB_ENSEMBLES__; i++)
	{
		...
		lin_comb(&M[i],A[i],B[i]); // Mi = aiP + biQ
	}
...
#pragma omp parallel private(R, a, a2, x, r, xDist, xDist_str, trail_length) shared(collision_count, x_res, trail_length_max) num_threads(nb_threads)
  {
    ...
    double_and_add(&R, P, a, E); // R = aP
    trail_length = 0;
		
    while(collision_count <= nb_collisions) 
      {
        if(is_distinguished(R, trailling_bits, &xDist)) // xDist = R.x >> trailling_bits
          {
            if(struct_add(a2, a, xDist, xDist_str)) // ajout de a dans la mémoire, a2 = a d'un autre point avec collision (si collision)
              {
                if(is_collision(x, a, a2, trailling_bits)) // si a et a2 forment une vraie collision
                  {
#pragma omp critical
                    {
                      collision_count++;
                      mpz_set(x_res, x); // on a x_res
                    }
                  }
              }
            mpz_urandomb(a, r_state, nb_bits);
          ...
          {
            r=hash(R.y); // y%20
            f(R, M[r], &R, E); // 1 étape (parmi 20) du chemin
            trail_length++;
